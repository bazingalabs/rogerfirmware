...
PORT = /dev/tty.usbserial-mc-nove1
UPLOAD_RATE = 57600
...
CXXINCS += $(shell find $(ARDUINO_LIB) -mindepth 1 -maxdepth 2 -type d ! -name examples -exec /bin/echo -n " -I{} " \;)
...
AVRDUDE_PORT = $(PORT)
AVRDUDE_ISP_PORT = usb
AVRDUDE_WRITE_FLASH = -U flash:w:applet/main.hex
AVRDUDE_WRITE_BOOTLOADER = -e -Ulock:w:0x3F:m -U lfuse:w:0xe2:m -U hfuse:w:0xda:m -U efuse:w:0x05:m -U flash:w:ATmegaBOOT_168_atmega328_pro_8MHz.hex

AVRDUDE_FLAGS = -C $(INSTALL_DIR)/hardware/tools/avr/etc/avrdude.conf \
-p $(MCU) -P $(AVRDUDE_PORT) -c $(AVRDUDE_PROGRAMMER) \
-b $(UPLOAD_RATE) -D

AVRDUDE_ISP_FLAGS = -C $(INSTALL_DIR)/hardware/tools/avr/etc/avrdude.conf \
-p $(MCU) -P $(AVRDUDE_ISP_PORT) -c $(AVRDUDE_ISP_PROGRAMMER) \
-b $(UPLOAD_RATE) -D
...
upload_isp: applet/main.hex
	$(AVRDUDE) $(AVRDUDE_ISP_FLAGS) $(AVRDUDE_WRITE_BOOTLOADER)
